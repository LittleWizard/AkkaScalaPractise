...
class Bot extends Actor with ActorLogging {
  ...
  val listingRegion = ClusterSharding(context.system).shardRegion(AuthorListing.shardName)
  ...
  var n = 0;
  val authors = Map(0 -> "Patrik", 1 -> "Martin", 2 -> "Ronald", 3 -> "Bjorn", 4 -> "Endre")
  ...
  val list: Receive = {
    case Tick =>
      listingRegion ! AuthorListing.GetPosts(currentAuthor)
